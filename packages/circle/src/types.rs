...
pub enum CircleError {
    ...
    NotImplemented = 38,
    ZeroPayoutAmount = 39,
    ContributionsExist = 40,
    ContributionTooHigh = 41,
...
// Emitted when a payout transfer fails, so clients can observe the failure
// even when the surrounding invocation succeeds (best-effort sweep paths).
#[contracttype]
#[derive(Clone, Debug)]
