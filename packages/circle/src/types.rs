...
// NOTE: new variants must only ever be APPENDED to this enum — the variant index is the
// on-chain XDR discriminant of every stored key, so inserting one would invalidate the
// state of already-deployed circles.
#[contracttype]
#[derive(Clone)]
pub enum DataKey {
    Circle,
    ...
    OracleContract,
    FallbackOracle,
    ContributionExists(Address, u32),
    VoteExists(Address, u32),
}
#[contracterror]
