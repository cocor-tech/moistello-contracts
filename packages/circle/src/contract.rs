...
use soroban_sdk::{Address,BytesN,Env,IntoVal,Vec};use crate::types::*;use crate::payout;use common::{math,pause};use common::reentrancy::ReentrancyGuard;
pub fn init(env:&Env,admin:&Address,factory:&Address,config:&CircleConfig)->Result<(),CircleError>{if config.max_members<2||config.contribution_amount<=0||config.total_rounds==0||config.payout_type>3||config.fee_bps>10_000{return Err(CircleError::InvalidAmount);}let circle=Circle{id:env.current_contract_address(),name:config.name.clone(),organizer:config.organizer.clone(),factory:factory.clone(),contribution_amount:config.contribution_amount,max_members:config.max_members,member_count:0,payout_type:config.payout_type,total_rounds:config.total_rounds,current_round:0,status:STATUS_PENDING,started_at:0,created_at:env.ledger().timestamp(),contribution_deadline_seconds:config.contribution_deadline_seconds,min_moi_score:config.min_moi_score,collateral_amount:config.collateral_amount,penalty_bps:config.penalty_bps,grace_period_seconds:config.grace_period_seconds,max_strikes:config.max_strikes,payout_bitmap:0,total_payouts:0,total_fees:0,slug:config.slug.clone()};env.storage().instance().set(&DataKey::Circle,&circle);env.storage().instance().set(&DataKey::Admin,admin);env.storage().instance().set(&DataKey::Factory,factory);env.storage().instance().set(&DataKey::FeeBps,&config.fee_bps);env.storage().persistent().set(&DataKey::Members,&Vec::<Member>::new(env));env.storage().persistent().set(&DataKey::Contributions,&Vec::<Contribution>::new(env));env.storage().persistent().set(&DataKey::Payouts,&Vec::<PayoutRecipient>::new(env));env.storage().persistent().set(&DataKey::Bids,&Vec::<AuctionBid>::new(env));env.storage().persistent().set(&DataKey::Votes,&Vec::<VoteEntry>::new(env));Ok(())}
pub fn join(env:&Env,member:&Address)->Result<(),CircleError>{pause::when_not_paused(env).map_err(|_|CircleError::ContractPaused)?;let _guard = ReentrancyGuard::new(env).map_err(|_| CircleError::NotActive)?;member.require_auth();let mut circle:Circle=env.storage().instance().get(&DataKey::Circle).ok_or(CircleError::NotInitialized)?;if circle.status==STATUS_DISPUTED||circle.status==STATUS_COMPLETED||circle.status==3u32{return Err(CircleError::NotActive);}let mut members:Vec<Member>=env.storage().persistent().get(&DataKey::Members).unwrap_or_else(||Vec::new(env));for i in 0..members.len(){if members.get(i).ok_or(CircleError::VecAccessError)?.address==*member{return Err(CircleError::AlreadyMember);}}if members.len()as u32>=circle.max_members{return Err(CircleError::CircleFull);}
    // #55/#54 — reputation-gated join: enforce min_moi_score, and the
    // joining member's tier-based circle-size/contribution ceilings.
    // Skipped entirely when no reputation registry is wired (set_reputation_registry
    // never called), so circles created before this feature keep working unchanged.
    check_reputation_gates(env,&circle,member)?;
    let now=env.ledger().timestamp();let pos=members.len()as u32;members.push_back(Member{address:member.clone(),position:pos,joined_at:now,strikes:0,status:MEMBER_ACTIVE,exited_at:0,total_contributions:0,total_received:0});circle.member_count=circle.member_count.checked_add(1).ok_or(CircleError::CircleFull)?;if circle.member_count>=circle.max_members&&circle.status==STATUS_PENDING{circle.status=STATUS_ACTIVE;circle.started_at=now;}env.storage().instance().set(&DataKey::Circle,&circle);env.storage().persistent().set(&DataKey::Members,&members);MemberJoined{member:member.clone(),position:pos}.publish(env);Ok(())}
pub fn contribute(env:&Env,member:&Address,amount:i128,round:u32)->Result<(),CircleError>{pause::when_not_paused(env).map_err(|_|CircleError::ContractPaused)?;let _guard = ReentrancyGuard::new(env).map_err(|_| CircleError::NotActive)?;member.require_auth();let circle:Circle=env.storage().instance().get(&DataKey::Circle).ok_or(CircleError::NotInitialized)?;if circle.status!=STATUS_ACTIVE{return Err(CircleError::NotActive);}if round!=circle.current_round{return Err(CircleError::RoundNotCurrent);}if amount!=circle.contribution_amount{return Err(CircleError::ContributionMismatch);}
    // #54 — re-check the contributor's tier-based contribution ceiling at
    // each contribution, not just at join: a member's tier can regress
    // (e.g. after record_default) between joining and a later round.
...
pub fn upgrade(env:&Env,admin:&Address,new_wasm_hash:&BytesN<32>)->Result<(),CircleError>{let stored_admin:Address=env.storage().instance().get(&DataKey::Admin).ok_or(CircleError::NotInitialized)?;if admin!=&stored_admin{return Err(CircleError::Unauthorized);}common::upgrade::upgrade_contract(env,admin,new_wasm_hash).map_err(|_|CircleError::Unauthorized)}
pub fn register_referral(env:&Env,referrer:&Address,referred:&Address,bonus_pct:u32)->Result<(),CircleError>{pause::when_not_paused(env).map_err(|_|CircleError::ContractPaused)?;let _guard=ReentrancyGuard::new(env).map_err(|_|CircleError::NotActive)?;referrer.require_auth();if referrer==referred{return Err(CircleError::SelfReferral);}let circle:Circle=env.storage().instance().get(&DataKey::Circle).ok_or(CircleError::NotInitialized)?;if circle.status!=STATUS_ACTIVE{return Err(CircleError::NotActive);}let members:Vec<Member>=env.storage().persistent().get(&DataKey::Members).ok_or(CircleError::NotInitialized)?;let mut referrer_found=false;let mut referred_found=false;for i in 0..members.len(){let m=members.get(i).ok_or(CircleError::VecAccessError)?;if m.address==*referrer{referrer_found=true;}if m.address==*referred{referred_found=true;}}if!referrer_found||!referred_found{return Err(CircleError::NotMember);}let mut referrals:Vec<Referral>=env.storage().persistent().get(&DataKey::Referrals).unwrap_or_else(||Vec::new(env));for i in 0..referrals.len(){let r=referrals.get(i).ok_or(CircleError::VecAccessError)?;if r.referred==*referred{return Err(CircleError::AlreadyReferred);}}if bonus_pct>1000{return Err(CircleError::InvalidBid);}referrals.push_back(Referral{referrer:referrer.clone(),referred:referred.clone(),bonus_pct,claimed:false});env.storage().persistent().set(&DataKey::Referrals,&referrals);ReferralRegistered{referrer:referrer.clone(),referred:referred.clone(),bonus_pct}.publish(env);Ok(())}
pub fn claim_referral_bonus(env:&Env,referrer:&Address,treasury:&Address)->Result<(),CircleError>{pause::when_not_paused(env).map_err(|_|CircleError::ContractPaused)?;let _guard=ReentrancyGuard::new(env).map_err(|_|CircleError::NotActive)?;referrer.require_auth();let circle:Circle=env.storage().instance().get(&DataKey::Circle).ok_or(CircleError::NotInitialized)?;if circle.status!=STATUS_ACTIVE{return Err(CircleError::NotActive);}let mut referrals:Vec<Referral>=env.storage().persistent().get(&DataKey::Referrals).ok_or(CircleError::NotInitialized)?;let mut bonus_total:i128=0;for i in 0..referrals.len(){let mut r=referrals.get(i).ok_or(CircleError::VecAccessError)?;if r.referrer==*referrer&&!r.claimed{let contributions:Vec<Contribution>=env.storage().persistent().get(&DataKey::Contributions).unwrap_or_else(||Vec::new(env));for j in 0..contributions.len(){let c=contributions.get(j).ok_or(CircleError::VecAccessError)?;if c.member==r.referred{let bonus=math::safe_mul(circle.contribution_amount,r.bonus_pct as i128).map_err(|_|CircleError::InvalidAmount)?;let bonus_div=math::safe_div(bonus,10000).map_err(|_|CircleError::InvalidAmount)?;bonus_total=math::safe_add(bonus_total,bonus_div).map_err(|_|CircleError::InvalidAmount)?;}}r.claimed=true;referrals.set(i,r);}}if bonus_total<=0{return Err(CircleError::InvalidAmount);}env.storage().persistent().set(&DataKey::Referrals,&referrals);ReferralBonusPaid{referrer:referrer.clone(),amount:bonus_total}.publish(env);Ok(())}
pub fn update_streak(env:&Env,member:&Address,round:u32)->Result<(),CircleError>{let circle:Circle=env.storage().instance().get(&DataKey::Circle).ok_or(CircleError::NotInitialized)?;if circle.status!=STATUS_ACTIVE{return Err(CircleError::NotActive);}let mut streaks:Vec<Streak>=env.storage().persistent().get(&DataKey::Streaks).unwrap_or_else(||Vec::new(env));let mut found=false;for i in 0..streaks.len(){let mut s=streaks.get(i).ok_or(CircleError::VecAccessError)?;if s.member==*member{s.last_round=round;s.current_streak=s.current_streak.checked_add(1).ok_or(CircleError::InvalidAmount)?;if s.current_streak>s.longest_streak{s.longest_streak=s.current_streak;}streaks.set(i,s.clone());found=true;StreakUpdated{member:member.clone(),current_streak:s.current_streak,longest_streak:s.longest_streak}.publish(env);break;}}if!found{streaks.push_back(Streak{member:member.clone(),current_streak:1,longest_streak:1,last_round:round});StreakUpdated{member:member.clone(),current_streak:1,longest_streak:1}.publish(env);}env.storage().persistent().set(&DataKey::Streaks,&streaks);Ok(())}
pub fn claim_streak_bonus(env:&Env,member:&Address,treasury:&Address)->Result<(),CircleError>{pause::when_not_paused(env).map_err(|_|CircleError::ContractPaused)?;let _guard=ReentrancyGuard::new(env).map_err(|_|CircleError::NotActive)?;member.require_auth();let circle:Circle=env.storage().instance().get(&DataKey::Circle).ok_or(CircleError::NotInitialized)?;if circle.status!=STATUS_ACTIVE{return Err(CircleError::NotActive);}let streaks:Vec<Streak>=env.storage().persistent().get(&DataKey::Streaks).ok_or(CircleError::NotInitialized)?;let mut streak_val:u32=0;for i in 0..streaks.len(){let s=streaks.get(i).ok_or(CircleError::VecAccessError)?;if s.member==*member{streak_val=s.current_streak;break;}}if streak_val<3{return Err(CircleError::InvalidStreakThreshold);}let bonus=math::safe_mul(circle.contribution_amount,streak_val as i128).map_err(|_|CircleError::InvalidAmount)?;let bonus_div=math::safe_div(bonus,100).map_err(|_|CircleError::InvalidAmount)?;StreakBonusPaid{member:member.clone(),amount:bonus_div,streak:streak_val}.publish(env);Ok(())}
pub fn get_referrals(env:&Env)->Vec<Referral>{env.storage().persistent().get(&DataKey::Referrals).unwrap_or_else(||Vec::new(env))}
pub fn get_streaks(env:&Env)->Vec<Streak>{env.storage().persistent().get(&DataKey::Streaks).unwrap_or_else(||Vec::new(env))}
pub fn get_member_streak(env:&Env,member:&Address)->Streak{let streaks:Vec<Streak>=env.storage().persistent().get(&DataKey::Streaks).unwrap_or_else(||Vec::new(env));for i in 0..streaks.len(){if let Some(s)=streaks.get(i){if s.member==*member{return s;}}}Streak{member:member.clone(),current_streak:0,longest_streak:0,last_round:0}}
