...
pub fn init(
    ...
    let circle = Circle {
        ...
        payout_cooldown_seconds: config.payout_cooldown_seconds,
...
/// - `Err(CircleError::RoundNotCurrent)` if provided round does not match current_round
/// - `Err(CircleError::PayoutCooldownActive)` if called before cooldown period has elapsed (#115)
/// - `Err(CircleError::ZeroPayoutAmount)` if computed net payout is zero or negative (#117)
...
pub fn trigger_payout(env: &Env, caller: &Address, round: u32) -> Result<(), CircleError> {
    ...
    // ── Issue #115: payout cooldown guard ────────────────────────────────────
    // Enforce minimum interval between consecutive payout calls.
    // `last_payout_timestamp == 0` means no payout has occurred yet — cooldown
    // is not applied for the very first payout of a circle.
    if circle.payout_cooldown_seconds > 0 && circle.last_payout_timestamp > 0 {
        let earliest_next = circle
            .last_payout_timestamp
            .checked_add(circle.payout_cooldown_seconds)
            .ok_or(CircleError::InvalidAmount)?;
        if env.ledger().timestamp() < earliest_next {
            return Err(CircleError::PayoutCooldownActive);
        }
    }
    ...
    // ── Issue #117: zero-net payout guard ────────────────────────────────────
    // Prevent transferring zero tokens and emitting a misleading payout event
    // when fee_bps is so high the net amount rounds down to zero or below.
    if net <= 0 {
    ...
    // ── Issue #115: record payout timestamp for next cooldown check ───────────
    circle.last_payout_timestamp = now;
