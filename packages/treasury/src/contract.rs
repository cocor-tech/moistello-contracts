...
/// # Panics
/// Never panics. All errors are returned as typed TreasuryError variants.
pub fn deposit(env:&Env,from:&Address,amount:i128,circle_id:&Address)->Result<(),TreasuryError>{pause::when_not_paused(env).map_err(|_|TreasuryError::ContractPaused)?;from.require_auth();if from!=circle_id{return Err(TreasuryError::Unauthorized);}if amount<=0{return Err(TreasuryError::InvalidAmount);}let token:Address=env.storage().instance().get(&DataKey::Token).ok_or(TreasuryError::NotInitialized)?;let token_client=soroban_sdk::token::Client::new(env,&token);token_client.transfer(from,&env.current_contract_address(),&amount);let bal:i128=env.storage().instance().get(&DataKey::Balance).unwrap_or(0);env.storage().instance().set(&DataKey::Balance,&bal.checked_add(amount).ok_or(TreasuryError::InvalidAmount)?);let mut deps:Vec<Deposit>=env.storage().persistent().get(&DataKey::Deposits).unwrap_or_else(||Vec::new(env));deps.push_back(Deposit{from:from.clone(),amount,circle_id:circle_id.clone(),timestamp:env.ledger().timestamp()});env.storage().persistent().set(&DataKey::Deposits,&deps);env.events().publish((env.current_contract_address(),symbol_short!("deposit")),FeeDeposited{from:from.clone(),amount,circle_id:circle_id.clone()});Ok(())}
...
/// # Panics
/// Never panics. All errors are returned as typed TreasuryError variants.
pub fn rescue_tokens(env:&Env,admin:&Address,to:&Address,token:&Address,amount:i128)->Result<(),TreasuryError>{admin.require_auth();let s:Address=env.storage().instance().get(&DataKey::Admin).ok_or(TreasuryError::NotInitialized)?;if admin!=&s{return Err(TreasuryError::Unauthorized);}if !pause::is_paused(env){return Err(TreasuryError::ContractNotPaused);}if amount<=0{return Err(TreasuryError::InvalidAmount);}let treasury_token:Address=env.storage().instance().get(&DataKey::Token).ok_or(TreasuryError::NotInitialized)?;if treasury_token==*token{let bal:i128=env.storage().instance().get(&DataKey::Balance).unwrap_or(0);if bal<amount{return Err(TreasuryError::InsufficientBalance);}env.storage().instance().set(&DataKey::Balance,&bal.checked_sub(amount).ok_or(TreasuryError::InsufficientBalance)?);}let token_client=soroban_sdk::token::Client::new(env,token);token_client.transfer(&env.current_contract_address(),to,&amount);env.events().publish((env.current_contract_address(),symbol_short!("rescue")),TokensRescued{to:to.clone(),token:token.clone(),amount});Ok(())}
