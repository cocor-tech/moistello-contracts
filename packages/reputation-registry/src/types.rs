...
#[contracttype]#[derive(Clone,Debug,PartialEq)]pub struct MoiScore{pub user:Address,pub score:u32,pub tier:u32,pub total_circles:u32,pub completed_circles:u32,pub defaulted_circles:u32,pub streak_count:u32,pub last_activity_at:u64,pub updated_at:u64}
...
#[contracttype]#[derive(Clone)]pub enum DataKey{Admin,Score(Address),UserActivityPage(Address,u32),UserActivityCount(Address),Streak(Address,Address),Completions(Address),MemberScore(Address),Defaults(Address),MemberLog(Address),LastRound(Address,Address)}
#[contracterror]#[derive(Debug,Clone,PartialEq,Eq)]pub enum ReputationError{NotInitialized=1,Unauthorized=2,ContractPaused=3,InvalidActivityType=4,ScoreNotFound=5,InvalidScoreImpact=6,Overflow=7}
