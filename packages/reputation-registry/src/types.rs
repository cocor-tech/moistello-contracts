...
#[contracttype]#[derive(Clone)]pub enum DataKey{Admin,Score(Address),UserActivityPage(Address,u32),UserActivityCount(Address),Streak(Address),Completions(Address),MemberScore(Address),Defaults(Address),MemberLog(Address)}
