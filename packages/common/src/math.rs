...
/// Returns the floor of the integer square root of a non-negative value.
///
/// The result is the largest `x` such that `x * x <= n` (Newton's method).
/// Negative inputs and zero both yield `0`.
pub fn isqrt(n: i128) -> i128 {
    if n <= 0 {
        return 0;
    }
    let mut x = n;
    // `n / 2` (floored, at least 1) instead of `(n + 1) / 2` so n = i128::MAX
    // cannot overflow; still converges to the floor sqrt.
    let mut y = (n / 2).max(1);
    while y < x {
        x = y;
        y = (x + n / x) / 2;
